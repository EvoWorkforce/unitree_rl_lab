...
class EventCfg:
    ...
    # debug: print pose error every 1 second
    # print_command = EventTerm(
    #     func=mdp.print_reset,
    #     mode="reset",
    # )

    # print_pose_error = EventTerm(
    #     func=mdp.print_pose_error,
    #     mode="interval",
...
class CommandsCfg:
    ...
    pose_command = mdp.UniformLevelPose2dCommandCfg(
        asset_name="robot",
        simple_heading=False,
        resampling_time_range=(60.0, 60.0),
        debug_vis=True,
        ranges=mdp.UniformLevelPose2dCommandCfg.Ranges(
            pos_x=(-0.5, 0.5), pos_y=(-0.5, 0.5), heading=(-math.pi, math.pi)
        ),
        limit_ranges=mdp.UniformLevelPose2dCommandCfg.Ranges(
...
class RewardsCfg:
    ...
    base_lin_vel_z = RewTerm(func=mdp.lin_vel_z_l2, weight=-0.1)
    # base_lin_acc = RewTerm(func=mdp.body_lin_acc_l2, weight=-1e-5)
    base_ang_vel_xy = RewTerm(func=mdp.ang_vel_xy_l2, weight=-1.0)
    # base_ang_acc_xy = RewTerm(func=mdp.ang_acc_xy_l2, weight=-1.0)
    base_lin_vel_limit = RewTerm(func=mdp.lin_vel_outside_range_exp, weight=-1.0, params={"v_min": 0.0, "v_max": 0.5, "std": 0.1})
...
class CurriculumCfg:
    ...
    pose_cmd_levels = CurrTerm(func=mdp.pose_cmd_levels)
...
class RobotPlayEnvCfg(RobotEnvCfg):
    def __post_init__(self):
        ...
        # spawn robot at world origin on reset
        self.events.reset_base.params["pose_range"] = {"x": (0.0, 0.0), "y": (0.0, 0.0), "yaw": (0.0, 0.0)}
        
        # use simple flat plane terrain centered at origin instead of terrain generator
