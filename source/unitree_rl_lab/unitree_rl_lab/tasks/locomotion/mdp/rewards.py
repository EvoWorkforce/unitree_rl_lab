...
"""Custom"""
# DISTANCE COMPASS (Linear Tracking Penalty)
# Calculates raw distance in meters between the hand and the goal.
# CRITICAL: Must use a NEGATIVE weight (e.g., -1.0). 
# The more negative the weight, the higher the penalty the agent suffers 
# every frame it stands idly far away. Acts as a compass guiding it toward the goal.
...
# BULLSEYE JACKPOT (Exponential Reaching Reward)
# Calculates a positive reward curve that explodes upward when entering the goal zone.
# CRITICAL: Must use a POSITIVE weight (e.g., 10.0) and an 'std' bubble size (e.g., 0.5m).
# It completely ignores the agent when far away, but hands out a massive 
# jackpot score spike as the hand approaches the exact center of the target.
